...
def cb_admin_check(func: Callable) -> Callable:
    async def decorator(client, cb):
        ...
        if cb.from_user.id in admemes:
            ...
            await cb.answer("𝗕𝗖 𝗗𝗨𝗦𝗥𝗘 𝗞𝗘 𝗣𝗟𝗔𝗬𝗟𝗜𝗦𝗧 𝗠𝗘𝗜 𝗖𝗛𝗘𝗗𝗞𝗛𝗔𝗡𝗜 𝗠𝗧 𝗞𝗥🤨!", show_alert=True)
            return
...
async def playlist(client, message):
    ...
    if not queue:
        await message.reply_text("Player 𝗶𝘀 𝗶𝗱𝗹𝗲🤑")
    temp = []
    for t in queue:
        temp.append(t)
    now_playing = temp[0][0]
    by = temp[0][1].mention(style="md")
    msg = "**𝗦𝗢𝗡𝗚 𝗖𝗛𝗔𝗟 𝗥𝗛𝗔❤️** in {}".format(message.chat.title)
    msg += "\n- " + now_playing
    msg += "\n- Req by " + by
    temp.pop(0)
    if temp:
        msg += "\n\n"
        msg += "**𝘁𝗵𝗼𝗱𝗮 𝘀𝗮𝗯𝗮𝗿 𝗸𝗿𝗼❤️**"
        for song in temp:
            name = song[0]
...
async def settings(client, message):
    ...
    if stats:
        ...
        await message.reply("𝗩𝗰 𝗮𝘀𝘀𝗶𝘀𝘁𝗮𝗻𝘁 𝗸𝗼 𝗮𝗱𝗱 𝗸𝗿🤨😒")
